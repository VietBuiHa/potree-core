...
class PotreeExtension extends Autodesk.Viewing.Extension {
    ...
    loadPointCloud(name, url, position) {
        if (!this._group) {
            this._initPointCloudGroup();
        }
        Potree.loadPointCloud(url, name, (e) => {
            const pointcloud = this._pointclouds[name] = e.pointcloud;
            pointcloud.isPotree = true; // for debugging
            if (position !== undefined) {
                pointcloud.position.copy(position);
            }
            pointcloud.material.size = 2;
            pointcloud.material.pointColorType = Potree.PointColorType.RGB; //RGB | DEPTH | HEIGHT | POINT_INDEX | LOD | CLASSIFICATION
            pointcloud.material.pointSizeType = Potree.PointSizeType.ADAPTIVE; //ADAPTIVE | FIXED
            pointcloud.material.shape = Potree.PointShape.CIRCLE; //CIRCLE | SQUARE
    ...
    _updatePointClouds() {
		const pointclouds = Object.values(this._pointclouds);
        if (pointclouds) {
    ...
    _initPointCloudGroup() {
        ...
        this.viewer.addEventListener(Autodesk.Viewing.CAMERA_CHANGE_EVENT, this._updatePointClouds.bind(this));
    }
}
